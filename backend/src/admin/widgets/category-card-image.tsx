...
const CategoryCardImageWidget = ({ data }: CategoryWidgetProps) => {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await updateCategoryMetadata(data.id, metadataSnapshot, {
        [CATALOG_CARD_IMAGE_URL_KEY]: uploadedUrl,
        [CATALOG_CARD_IMAGE_ALT_KEY]: altText || imageFallbackAlt(data),
      }, setMetadataSnapshot)
      ...
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <Text size="small" className="text-ui-fg-muted">
            Upload an image that will show on the storefront category grid.
          </Text>
        </div>
      </div>
      <div className="space-y-4">
        {preview ? (
          <div className="overflow-hidden rounded-xl border border-ui-border-subtle">
            ...
            <img src={preview} alt={altText || imageFallbackAlt(data)} className="h-56 w-full object-cover" />
          </div>
        ) : (
        ...
        <div className="flex flex-col gap-2">
          <Label htmlFor="category-card-image">Upload image</Label>
          <input
            ref={fileInputRef}
        ...
        <div className="flex flex-col gap-2">
          <Label htmlFor="category-card-alt">Alt text</Label>
          <Input
            id="category-card-alt"
            value={altText}
            onChange={(event) => setAltText(event.target.value)}
            disabled={isBusy}
            placeholder={`E.g. ${data.name} collage`}
          />
          <Text size="small" className="text-ui-fg-muted">
...
const updateCategoryMetadata = async (
  id: string,
  ...
  if (!response.ok) {
    throw new Error("Unable to persist category image")
  }

  setSnapshot(nextMetadata)
}

const imageFallbackAlt = (category: AdminProductCategory) => `${category.name} preview`

export const config = defineWidgetConfig({
  zone: "product_category.details.side.after",
})

export default CategoryCardImageWidget
